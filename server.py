...
def _machine_substrate_health() -> dict[str, Any]:
    ...
    try:
        ...
        band = "FRESH" if age < 3600 else "AGED" if age < 14400 else "STALE"
        return {
...
def well_attest_to_kernel(
    ...
    """
    return _wt_well_attest_to_kernel(
        attestation_kind=attestation_kind, actor_id=actor_id, ctx=ctx
    )
...
def _well_assess_machine_telemetry() -> dict[str, Any]:
    ...
    for res_name, psi_key in [
        ("Memory", "memory_full_avg60"),
        ("I/O", "io_full_avg60"),
    ]:
        full_val = pressure.get(psi_key, 0)
        if full_val > 1:
            issues.append(
                f"{res_name} FULL pressure CRITICAL: {psi_key}={full_val}% -- all tasks stalled"
            )
    ...
    if entropy_avail is not None:
        if entropy_avail < 100:
            issues.append(
                f"Entropy CRITICAL: {entropy_avail} bits -- TLS/SSH operations may hang"
            )
        elif entropy_avail < 200:
            warnings.append(
                f"Entropy LOW: {entropy_avail} bits -- cryptographic operations may be slow"
            )
    ...
    async def _governance_call_tool(name, arguments=None, **kwargs):
        ...
        try:
            ...
            _meta = (
                ...
                and isinstance(arguments.get("_meta"), dict)
                else None
            )
...
def well_registry_status(
    ...
    known_aliases = {
        ...
        "well_444_gateway",
        "well_000_init",
        # Triad phase tools (registered, internal — 2026-08-21 registry reconciliation)
        "well_assess_triadic_state",
        "well_attest_to_kernel",
        "well_classify_machine_state",
        "well_consent_audit",
        "well_consent_set_scope",
        "well_frame_read_snapshot",
        "well_get_triadic_snapshot",
    ...
    exported_surface = sorted(
        ...
        if n not in known_aliases and n not in LEGACY_ALIASES
    )
