...
def test_sovereign_entropy_current():
    """Test that current mode returns valid entropy score."""
    from server import well_assess_sovereign_entropy

    result = _unwrap(asyncio.run(well_assess_sovereign_entropy(mode="current")))
...
def test_sovereign_entropy_protect():
    """Test that protect mode returns recommendations."""
    from server import well_assess_sovereign_entropy

    result = _unwrap(asyncio.run(well_assess_sovereign_entropy(mode="protect")))
    assert "protections" in result
    assert "current_entropy" in result


def test_sovereign_entropy_assess():
...
def test_sovereign_entropy_low():
